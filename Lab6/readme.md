...
![Схема сети, используемая в данной лабораторной работе](./img/topology_lab6.png)
...
| **C3** | 172.22.3.33/24 | 172.22.3 |
| **C4** | 172.22.4.44/24 | 172.22.4.1 |
| **C5** | 172.22.2.55/24 | 172.22.2.1 |
| **C6** | 172.22.4.66/24 | 172.22.4.1 |
...
#### Настраиваем VRF для L3 VNI

На всех Leaf создаем `vrf instance L3VNI` и включаем в нем маршрутизацию `ip routing vrf L3VNI`.

#### Настраиваем клиентские VLAN

Добавляем на коммутаторах клиентские VLAN, назначем их в vrf L3VNI и сздаем на них SVI с виртуальным IP согласно таблице ниже:

| VLAN ID | Устройство | Подключенные клиенты | Virtual IP |
|---|---|---|---|
| 101 | L1 | C1 | 172.22.1.1/24 |
| 102 | L1,L2 | C2,С5 | 172.22.2.1/24 |
| 103 | L3 | C3 | 172.22.3.1/24 |
| 104 | L1,L3 | C3,C6 | 172.22.4.1/24 |

Определяем на всех Leaf коммутаторах единый MAC адрес виртуального маршрутизатора для Anycast gateway: `ip virtual-router mac-address 02:00:00:00:1e:af`.

Добавляем клиентские порты как access к соответствующим VLAN.

#### Настраиваем VxLAN интерфейс:

К интерфейсу Vxlan 1, добавляем клиентские VLAN и привязываем их к VNI согласно следующей таблице. Так же на всех коммутаторах добавляем к Vxlan 1 vrf L3VNI:
...
| L3 | 103 | 10103 |
| L3 | 104 | 10104 |
| L1,L2,L3 | L3VNI | 90000 |



#### Настраиваем BGP EVPN

К  существующей конфигурации router bgp на Leaf коммутаторах добавляем недостающие MAC-IP VRF, настраиваем RD и RT согласно таблице:

| Коммутатор | VLAN | RD | RT |
|---|---|---|---|
| L1 | 101 | 65501:101 | both 65500:10101 |
| L1 | 102 | 65501:102 | both 65500:10102 |
| L2 | 102 | 65502:102 | both 65500:10102 |
| L2 | 104 | 65502:104 | both 65500:10104 |
| L3 | 103 | 65503:103 | both 65500:10103 |
| L3 | 104 | 65503:104 | both 65500:10104 |

Включим `redistribute learned` для каждого MAC-IP VRF.

Так же на каждом коммутаторе к конфигурации router bgp добавим `vrf L3VNI`, соконфигурируем для него `rd 65503:1` и `route-target 6500:90000`, а так же вклюим опцию `redistribute connected`.
